...
function attr(raw, key) {
  ...
  const m = re.exec(raw || ''); return decodeHtml(m ? (m[1] ?? m[2] ?? m[3] ?? '') : '');
}
...
async function domainManifest() {
  ...
  try {
    const r = await text(DOMAIN_MANIFEST, '', 8000);
    manifestCache = r.code === 200 ? JSON.parse(r.text) : { sources: {} };
...
function anchorRows(html, base) {
  ...
  for (const m of String(html || '').matchAll(re)) {
    ...
    const href = attr(raw, 'data-url') || attr(raw, 'data-streamx') || attr(raw, 'href');
    if (!href) continue;
    const player = http(abs(href, base)); if (!player) continue;
    const nameMatch = /class\s*=\s*["'][^"']*(?:name|home|channel-name)[^"']*["'][^>]*>([\s\S]*?)<\//i.exec(body);
    const title = stripTags(nameMatch?.[1] || attr(/<img\b([^>]*)>/i.exec(body)?.[1] || '', 'alt') || body);
    if (!title) continue;
...
function extractPlayerUrls(html, playerUrl) {
  ...
  if (base) {
    const id = q(playerUrl, 'id');
    if (/privateStream\s*=\s*1\b/i.test(html)) add(base);
    else if (id && /\/playlist\.m3u8/i.test(html)) add(`${base.replace(/\/$/, '')}/${encodeURIComponent(id)}/playlist.m3u8`);
  }
  return out;
}
...
async function genericSource(source, wioChannel, matches) {
  const manifest = await domainManifest();
  const entry = manifest?.sources?.[source.key] || {};
  ...
  for (const root of roots) {
    try {
      ...
      const rows = anchorRows(home.text, home.url);
      const selected = rows.filter(row => matches(wioChannel, row)).slice(0, 4);
      const found = [];
...
async function domino(wioChannel, matches) {
  const api = 'http://dominotv58.site/admin_panel/api/';
  const categories = ['⚽ 🇹🇷 SPOR', '⚽ Sports'];
...
export async function getLegacyStreams(wioChannel, matches) {
  const tasks = [domino(wioChannel, matches), ...SOURCES.map(source => genericSource(source, wioChannel, matches))];
  const settled = await Promise.allSettled(tasks);
  const out = [];
  for (const item of settled) if (item.status === 'fulfilled') out.push(...item.value);
